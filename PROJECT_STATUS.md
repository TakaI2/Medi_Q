...
1. **T2-015**: Google Calendar関連コード削除（優先度: 高）
   - 既存のGoogle Calendar APIコードを完全に削除し、新しいSQLiteベースシステムに移行

2. **T2-018**: 型定義更新（types/index.ts）（優先度: 高）
   - Prismaスキーマに対応した型定義を整備
...
1. **Phase 2-4の並行開発**
   - 複数フェーズが同時進行中のため、依存関係の管理が重要
   - 特にT2-018（型定義更新）は多くのタスクに影響
...
- Phase 2の残タスク（T2-015〜T2-019）を優先的に完了させる
...
#### 未着手 ⚪
- ⚪ T2-015: Google Calendar関連コード削除（優先度: 高）
...
4. ⚪ **T3-018**: 受付処理API（受付画面改修の前提）← **最重要**
5. ✅ T4-017 → T4-020: カレンダーUI実装（完了）
