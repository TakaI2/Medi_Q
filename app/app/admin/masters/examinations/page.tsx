...
export default function ExaminationsPage() {
  ...
  const fetchExaminations = async () => {
    try {
      const res = await fetch('/api/masters/examinations');
      const data: ApiResponse<Examination[]> = await res.json();
  ...
  const handleSave = async () => {
    ...
    try {
      const url = editingId
        ? `/api/masters/examinations/${editingId}`
        : '/api/masters/examinations';
  ...
  const handleDelete = async (id: number, name: string) => {
    ...
    try {
      const res = await fetch(`/api/masters/examinations/${id}`, {
        method: 'DELETE',
      });
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">検査項目マスタ管理</p>
              </div>
            </div>
            <button
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          <div className="p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-gray-900">検査項目一覧</h2>
              <span className="text-sm text-gray-500">{examinations.length}件</span>
            </div>
            ...
            <div className="p-6">
              ...
              <div className="mb-4">
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  検査項目名 <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  value={name}
