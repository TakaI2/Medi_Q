...
export function generateVoiceText(
  ...
  waitingArea: string,
  doctor: string,
  hasExamination: boolean = true
): string {
  const guidance = hasExamination
    ? VOICE_TEMPLATES.guidance
        .replace('{department}', department)
        .replace('{waitingArea}', waitingArea)
        .replace('{doctor}', doctor)
    : `${waitingArea}前にお越しください。${doctor}先生が担当します。`;
...
export const ERROR_MESSAGES = {
  ...
  CALENDAR_ERROR:
    '診察予定の取得に失敗しました。しばらくしてから再度お試しください。',
